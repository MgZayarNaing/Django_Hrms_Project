...
class OrderBy(View):
    ...
    def get(self, request):
        ...
        paginator = Paginator(departments, 10) # Show 2 contacts per page.
...
class AllDepartments(LoginRequiredMixin,View):
	...
	def get(self, request):
		...
		paginator = Paginator(all_departments, 10 ) # Show 2 contacts per page.
		page_number = request.GET.get('page')
		page_obj = paginator.get_page(page_number)
...
class DeleteDepartment(PermissionRequiredMixin,View):
	...
	# def post(self, request, department_id):
	# 	print('DeleteDepartment POST call ++++++++++++++')
